...
try:
    ...
    if team_api_response.status_code == 200:
        team_data = team_api_response.json() #converts from json data to python dictionary representation of data

        teams_df = pd.DataFrame.from_dict(team_data) #converts to data frame
        ...
        selected_league = st.sidebar.selectbox('Pick league ID', unique_leagues) #creates a select box to select league id
